...
//                }
                /*
                 * 将获取到的图片路径（http://localhsot:8080/umed/jsp/upload/XXX.jpg）
                 * 通过字符串截取将（ http://localhsot:8080） 截取掉
                 * */
                var src=node.src;
                var arrsrc = src.split("/");
                var newsrc = "";
                for (var i = 3; i < arrsrc.length; i++) {
                	newsrc+="/"+arrsrc[i];
				}
                src = newsrc;
                return arr.push({
        ...
        callback: function (editor, $w, url, state) {
            ...
            if (state == "SUCCESS") {
                ...
                //将（editor.options.imagePath） 改为 本项目的路径
                var $img = $("<img src='./jsp/" + url + "' class='edui-image-pic' />"),
