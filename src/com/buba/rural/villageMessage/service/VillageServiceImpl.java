...
public class VillageServiceImpl implements IVillageService{
		...
		public String updated(Country_message[] country,Country_message countrymessage) {
			...
			try {
				vrUtil.updateVersionRecord(country, "update", "1", accs.getUsername(), df.format(new Date()), "id");
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		public String addVillage(Country_message countrymessage) {
			...
			//操作人
			Acc accs=(Acc) ServletActionContext.getRequest().getSession().getAttribute("resultacc");
			countrymessage.setOperator(accs.getUsername());
			try {
				vrUtil.insertVersionRecord(countrymessage, "add", "1", accs.getUsername(), df.format(new Date()), "id");
			} catch (Exception e) {
				e.printStackTrace();
			}
