...
public class FinanceServiceImpl implements IFinanceService{
	...
	public String updated(Country_group_finance[] country,Country_group_finance groupfinance) {
		...
		try {
			vrUtil.updateVersionRecord(country, "update", "7", accs.getUsername(), df.format(new Date()), "id");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String addFinance(Country_group_finance groupfinance) {
		...
		//操作人
		Acc accs=(Acc) ServletActionContext.getRequest().getSession().getAttribute("resultacc");
		groupfinance.setOperator(accs.getUsername());
		try {
			vrUtil.insertVersionRecord(groupfinance, "add", "7", accs.getUsername(), df.format(new Date()), "id");
		} catch (Exception e) {
			e.printStackTrace();
		}
